...
public:
	...
	void exploreTop(int vertex, list<int> &linearOrder)
	{
		visited[vertex] = true;
		cout << adjList[vertex].size() << " ";
		cout << vertex << endl;
		if (adjList[vertex].size())
		{
			for (int i = 0; i < adjList[vertex].size(); i++)
			{
				int value = adjList[vertex][i];
				if (!visited[value])
					exploreTop(value , linearOrder);
			}
		}
		// this is postvisit
		linearOrder.push_front(vertex);
	}
	void topologicalSort(){
		list<int> linearOrder;
		for (int u = 0; u < adjList.size(); u++){
			if (!visited[u])exploreTop(u, linearOrder);
		}
		for (auto value : linearOrder)
